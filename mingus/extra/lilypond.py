...
def from_Note(note, process_octaves=True, standalone=True):
    ...
    if not isinstance(note, TemporalNote):
        if(note[2] is not None):
            ...
            o = 3
        d = int(note[1])
        note = TemporalNote(name=n, octave=o, duration_denominator=d)
...
def from_Bar(bar, showkey=True, showtime=True):
    ...
    for note in bar.bar:
       result = result + " " + from_Note(note, standalone = False) + str(note[1])
    #result = ' '.join(['{}{}'.format(from_Note(note, standalone=False), int(note[1])) for note in bar.bar])
    #result = "{ %s }"%result if bar.temporal_notes else ""
    if (hasattr(bar, 'chords')):
        result = "\\chords { %s } %s"%(' '.join([from_Chord(chord_notes) for chord_notes in bar.chords]),
                                       result)

    # Process the time
    if showtime:
        result =  '\\numericTimeSignature \\time %d/%d %s' % (bar.meter[0], bar.meter[1], result)
    if showkey:
   	    result = '\\key %s %s' % (convert_Key(bar.key), result)
...
def from_Track(track):
    ...
    for bar in track.bars:
        ...
        if lasttime != bar.meter:
            ...
            showtime = False
        result += from_Bar(bar, showkey or first, showtime or first) + ' '
        lastkey = bar.key
        lasttime = bar.meter
        first = False
    return '{ %s }' % result
...
def to_svg(ly_string, filename):
    """Save a string in LilyPond format to a SVG.

    LilyPond in the $PATH is needed.
    """
    return save_string_and_execute_LilyPond(ly_string, filename, '-dbackend=svg')
...
def save_string_and_execute_LilyPond(ly_string, filename, command):
    ...
    p = subprocess.Popen(command, shell=True, stdout=devnull, stderr=devnull).wait()
